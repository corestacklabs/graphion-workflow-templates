# Graphion™ Pipeline — Full SBOM Workflow
...
This repository contains a reusable GitHub action pipeline (`.github/workflows/sbom.yml`) to create, scan, and publish an SBOM to CoreStack in a single call. In addition, for container images, the pipeline will lint the dockerfile with Hadolint and scan the image with Dockle.

> **Internal note:** `.github/workflows/sbomdev.yml` also exists alongside `sbom.yml` — it's an unstable in-progress copy used to test pipeline changes before they land in `sbom.yml`. It is not documented here and should not be called by customer/client workflows; it can change or break without notice.

See [README.md](README.md) for an overview of every workflow in this repo, including the split `create-sbom.yml` → `scan-sbom.yml` → `upload-sbom.yml` pipeline that lets you enable/disable individual stages instead of calling one monolithic workflow. See [CHANGELOG.md](CHANGELOG.md) for behavior changes that may need action from existing callers.
